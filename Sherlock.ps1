# Modified version of https://github.com/Maelstromage/Log4jSherlock
# Over-rides to scan only for C
# Over-rides so that it only scans local drive
...
function get-menu{
    ...
    do{
        ...
        #MODIFIED to harcode in the mode
        #$readhost = Read-Host
        $Readhost = "R"
        switch ($readhost){
            'U' {
...
function Scan-MultipleSystems{
    ...
    foreach ($comp in $comps){
        ...
        if ($creds -ne $null){
            ...
            #MODIFIED TO NOT SCAN MULTIPLE SYSTEMS
